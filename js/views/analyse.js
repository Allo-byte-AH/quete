...
var VueAnalyse = (function () {
  ...
  function persister() {
    try { localStorage.setItem(CLE, JSON.stringify({ f: f, colonnes: colonnes, tri: tri })); } catch (e) {}
  }
  ...
  function panneauFiltres() {
    ...
    var cls = State.clients().filter(function (c) { return !c.archive; })
      .concat([{ id: Analyse.SANS, nom: 'Sans client', couleur: '#4a4f60' }]);
  ...
  function tousLesIds(groupe) {
    ...
    if (groupe === 'clients') return State.clients().filter(function (c) { return !c.archive; }).map(function (c) { return c.id; }).concat([Analyse.SANS]);
